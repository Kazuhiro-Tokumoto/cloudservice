...
- **内部メール**: ユーザー名を宛先にしたメール(件名・本文・添付ファイルとも
  E2E 暗号化、引用付き返信対応)
- **プッシュ通知**: 新着メール・ファイル共有を受けるとスマホ等に Web Push 通知
  (画面右上の「通知オフ」ボタンで有効化。E2E 暗号化のため通知に中身は載らない)
- **スマホ対応**: レスポンシブ UI でスマホのブラウザからそのまま使える
...
| POST | `/api/mail` | メール送信(暗号化済み件名・本文・添付 + 包んだ鍵×2) |
| GET | `/api/push/vapid` | プッシュ通知の VAPID 公開鍵 |
| POST | `/api/push/subscribe` | プッシュ通知の購読登録 |
